...
class LSelectedFlow @Inject constructor(
    private val appDataStore: AppDataStore,
    private val dataStoreKeys: DataStoreKeys,
) : FlowAction<Unit, Boolean>() {
    override fun Unit.createFlow(): Flow<Boolean> =
        appDataStore.get(dataStoreKeys.lSelected).map { waterUnitFromPhone ->
            waterUnitFromPhone ?: true //if from phone is null it returns true
        }
}